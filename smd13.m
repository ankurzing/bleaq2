function [ulEliteFunctionValue, llEliteFunctionValue, ulEliteIndiv, llEliteIndiv, ulFunctionEvaluations, llFunctionEvaluations,llCalls,gen,ulDim,llDim,ulPopSize,llPopSize] = smd13(ulPopSize, llPopSize, ulMaxGens, llMaxGens, ulDim, llDim)

problemName = 'smd13';             % Test problem name
...
ulDimMin = [-5*ones(1,p)  -10*ones(1,r)];  % Minimum bound accross LL dimensions
ulDimMax = [10*ones(1,p)  2.3026*ones(1,r)];  % Maximum bound accross LL dimensions

eps = 0.00001;
llDimMin = [-5*ones(1,q)  eps*ones(1,r)];  % Minimum bound accross LL dimensions
llDimMax = [10*ones(1,q)  10*ones(1,r)];  % Maximum bound accross LL dimensions
